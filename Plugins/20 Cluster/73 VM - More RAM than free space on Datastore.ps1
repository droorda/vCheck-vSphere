...
Foreach($Machine in $VM | Where {$_.PowerState -eq "PoweredOn"}) {
    ...
    $Details.FreeSpaceMB = ((get-vm $Machine).DatastoreIdList | foreach {get-datastore -id $_ } |Where {$_.Name -eq (($Machine.ExtensionData.Config.Files.VmPathName).Split('[')[1]).Split(']')[0]}).FreeSpaceMB
    $vmInfo += $Details
}
